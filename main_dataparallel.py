...
class Model_trainer_predictor(object):
    ...
    def train(self, save_path, batchsize, seq_len, max_time, 
                              clip_threshold=8, print_interval=10000):
        
        models = [self.model.to_gpu(i) for i in range(4)]
            
        optimizer = optimizers.Adam()
        optimizer.setup(self.model.function_set)

        Generator = self.corpus.SequenceGenerator(seq_len, batchsize, no_iterations_per_epoch,
                               yield_labels=True)
...
#%%
Corpus = corpus_class(
            corpus_path='/home/bordingj/data/findZebra_corpus.pkl', subset_ratio=1/100)
...
#%%
trainer.train('test.pkl',  batchsize=200, seq_len=50, no_iterations=5000, clip_threshold=8,
