...
class Function(object):
    ...
    All function implementations defined in :mod:`nula.functions` inherit
    this class.
    ...
    def copy(self):
        ...
        for name, value in self.__dict__.items():        
            if name not in self.parameter_names and \
	       name not in self.gradient_names and hasattr(value, 'shape'):
                setattr(a_copy, name, value.copy())
        return a_copy

    def __call__(self, *inputs, copy_func=True):
